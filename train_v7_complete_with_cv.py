"""
完整的V7模型训练流程（包含5折交叉验证）
基于75样本生成的10万数据（来港时长1-48个月）
预计耗时：1-2小时
"""
import pandas as pd
import numpy as np
...
print("=" * 80)
print("V7 Ultimate模型完整训练流程（包含5折交叉验证）")
print("基于75样本生成的10万数据（来港时长1-48个月）")
print("预计耗时：1-2小时")
print("=" * 80)
...
print(f"✓ 5折交叉验证设置完成")
print(f"✓ 每折训练集: {int(len(X_full) * 0.8):,} 样本")
print(f"✓ 每折验证集: {int(len(X_full) * 0.2):,} 样本")
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(X_full), 1):
    ...
    print(f"\nFold {fold}/5 完成 - 耗时: {(time.time() - start_time)/60:.1f}分钟")

# ============================================================================
# 第8步：交叉验证结果汇总
# ============================================================================
print("\n" + "=" * 80)
print("【步骤8/12】5折交叉验证结果汇总")
print("=" * 80)
...
shap_values_dict = {}
for model_name, model in [('xgb', model_for_shap['xgb']), 
...
# 加权平均
weights = np.array([0.167, 0.167, 0.167, 0.166, 0.166])
shap_values_ensemble = (
    weights[0] * shap_values_dict['xgb'] +
    weights[1] * shap_values_dict['lgb'] +
    weights[2] * shap_values_dict['cb'] +
    weights[3] * shap_values_dict['gbm'] +
    weights[4] * shap_values_dict['rf']
)
...
【保存的文件】
  1. cv_results_75samples.csv - 交叉验证结果
  2. shap_values_75samples_cv.npy - SHAP值
  3. feature_importance_75samples_cv.csv - 特征重要性
  4. v7_models_75samples_5fold.pkl - 5个fold的模型
